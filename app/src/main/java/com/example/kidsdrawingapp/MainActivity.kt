...
class MainActivity : AppCompatActivity() {
    ...
    //request permission for access of external storage
    ...
    private fun getBitmapFromView(view: View):Bitmap{
        ...
        //putting returned bitmap into the canvas
        val canvas=Canvas(returnedBitmap)
        val bgDrawable=view.background
        //if there is a background add it to the canvas
        if(bgDrawable!=null){
            bgDrawable.draw(canvas)

        }else
        //if there is no background add white bg
            {
            ...
            if(mBitmap!=null){
                try{
                    ...
                    val f= File(externalCacheDir?.absoluteFile.toString()
                            +File.separator + "KidsDrawingApp" +System.currentTimeMillis()/1000
                            +".png")
